...
#! modelnet40
def modelnet40_load_data(train, use_normals):
	if train: partition = 'train'
	else: partition = 'test'
	BASE_DIR = os.path.dirname(os.path.abspath(__file__))
	DATA_DIR = '/media/user/sda3/learning3d/data'
	all_data = []
	all_label = []
...
#! NIA data loader
def NIA_load_data(train, use_normals):
	print("dataloading...")
	DATA_DIR_lable = os.path.join(os.getcwd(),'data/NIA')
	DATA_DIR = os.path.join(os.getcwd(),'data/new_NIA')
	all_paths = []
	all_label = []
	
	#! label loader
	labels = []
	for root, dirs, files in os.walk(DATA_DIR_lable):
		dirs.sort()
		for file in files:
			if file.split('.')[-1] == 'xlsx':
				metadata = pd.read_excel(os.path.join(root,file))
				for i in range(len(metadata)-1):
					labels.append(metadata[i+1:i+2]['파일명'][i+1])
					labels.append(metadata[i+1:i+2]['중분류(분야)'][i+1])

	datalist = open("dataset.txt", 'r')
	datalist = datalist.read().splitlines()
	for i in range(int(len(datalist))):
		datalist[i] = datalist[i].split(' ')[0]

	#! data loader and pair
		...
		for file in files:
			if file.split('.')[-1] == 'xyz':
				...
				if file_name in labels:
					...
					num = labels.index(file_name)
					if '/' in labels[num+1]:
						labels[num+1] = labels[num+1].split('/')[0]

					if labels[num+1] == '조각':		all_label.append(0)
					elif labels[num+1] == '공예':	all_label.append(1)
					elif labels[num+1] == '건축':	all_label.append(2)
					elif labels[num+1] == '악기':	all_label.append(3)
				else:
					print(file_name)

	#! class
	if train:
		class1 = int(all_label.count(0))
		class2 = int(all_label.count(1))
		class3 = int(all_label.count(2))
		class4 = int(all_label.count(3))

		num1 = math.ceil(class1/5)
		num2 = math.ceil(class2/5)
		num3 = math.ceil(class3/5)
		num4 = math.ceil(class4/5)
		...
		for i in range(len(all_label)):
			...
			if num1 == 0 and num2 == 0 and num3 == 0 and num4 == 0:
				all_label = [item for item in all_label if item != 'x']
				all_paths = [item for item in all_paths if item != 'x']
				break
	else:
		#! test
		# class1 = int(all_label.count(0))
		...
		# num1 = math.ceil(class1/10)
		# num2 = math.ceil(class2/10)
		# num3 = math.ceil(class3/10)
		# num4 = math.ceil(class4/10)
		...
		num1 = math.ceil(class1/2)
		num2 = math.ceil(class2/2)
		num3 = math.ceil(class3/2)
		num4 = math.ceil(class4/2)
	...
	#! 1024 points
	if train:
		print('training dataset -> 조각: %d, 공예: %d, 건축: %d, 악기: %d' % (all_label.count(0),all_label.count(1), all_label.count(2), all_label.count(3)))
		print("총 data: %d장" % len(all_paths))
		with open("train_dataset.txt", "w") as file:
			for i in range(int(len(all_paths))):
				file.write("%s \n" % all_paths[i].split('/')[-2])
	else:
		print('testing dataset -> 조각: %d, 공예: %d, 건축: %d, 악기: %d' % (all_label.count(0),all_label.count(1), all_label.count(2), all_label.count(3)))
		print("총 data: %d장" % len(all_paths))
		with open("test_dataset.txt", "w") as file:
			for i in range(int(len(all_paths))):
				file.write("%s \n" % all_paths[i].split('/')[-2])
		# for i in range(int(len(all_paths))):
			# test = list(all_paths[i])
			# test[-3:] = 'obj'
			# test = ''.join(test)
			# shutil.copy(all_paths[i], 'test_dataset_xyzfile/%s' % all_paths[i].split('/')[-1])

	return all_paths, all_label
...
class ModelNet40Data(Dataset):
	def __init__(
		...
		self.data, self.labels = modelnet40_load_data(train, use_normals)
...
class NIAData(Dataset):
	def __init__(
		...
		super(NIAData, self).__init__()
		if load:
			self.paths, self.labels = NIA_load_data(train, train)
			self.num_points = num_points
			self.randomize_data = randomize_data

	def __getitem__(self, index):
		vert = np.loadtxt(self.paths[index])
		if int(self.paths[index].split('.')[0].split('d')[-1]) > 6000:
			vert = vert * 100
		try:
			data = vert

			if len(data.shape) > 1:
				choice = np.random.choice(data.shape[0], 1024, replace=True)
				data = data[choice]
			else:
				print('Warning: No vertex in {}. Continue...'.format(self.paths[index]))
		except:
			print('Warning: No vertex in {}. Continue...'.format(self.paths[index]))
...
class ClassificationData(Dataset):
	def __init__(self, data_class=NIAData()):
	...
	def __getitem__(self, index):
		...
		return (self.data_class.data[index], self.data_class.labels[index])


class RegistrationData(Dataset):
	def __init__(self, algorithm, data_class=NIAData(), partial_source=False, partial_template=False, noise=False, additional_params={}):
